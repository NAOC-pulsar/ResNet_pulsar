...
# restore_checkpoint = ''
restore_checkpoint = os.path.join(checkpoint_path, 'model_epoch1.ckpt')
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
    ...
    for i in range(val_batches_per_epoch):
        ...
        pre_loss, pre_acc, pre_y = sess.run([cost, accuracy, output], feed_dict={x: batch_validx, y: batch_validy})

        v_loss += pre_loss
        v_acc += pre_acc
        count += 1
        y_true = np.argmax(batch_validy, 1)
        y_pre = np.argmax(pre_y, 1)
        if i == 0:
            con_matrix = confusion_matrix(y_true, y_pre)
        else:
            con_matrix += confusion_matrix(y_true, y_pre)
