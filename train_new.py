...
# restore_checkpoint = ''
restore_checkpoint = os.path.join(checkpoint_path, 'model_epoch7.ckpt')
...
hps = resnet_model_new.HParams(batch_size=batch_size,
...
# Initialize model
model = resnet_model_new.ResNet(hps, x, y)

# Link variable to model output
tmp = model.logits
output = tf.nn.softmax(tmp, name='output')
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(val_batches_per_epoch):
            ...
            valid_loss, valid_acc, valid_out = sess.run([cost, accuracy, output], feed_dict={x: batch_validx, y: batch_validy})
